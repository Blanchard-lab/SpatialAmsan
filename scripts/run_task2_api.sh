...
# ---------- Task 2 API evaluation ----------

run_task "Task2 API  [openai / ${OPENAI_MODEL}]" \
  python ./task2_eval/apis/evaluate_task2.py \
    --api openai \
    --problems "${PROBLEMS}" \
    --frames_dir "${FRAMES_DIR}" \
    --output "./results/task2_openai.json" \
...
run_task "Task2 API  [claude / ${CLAUDE_MODEL}]" \
  python ./task2_eval/apis/evaluate_task2.py \
    --api claude \
    --problems "${PROBLEMS}" \
    --frames_dir "${FRAMES_DIR}" \
    --output "./results/task2_claude.json" \
...
run_task "Task2 API  [gemini / ${GEMINI_MODEL}]" \
  python ./task2_eval/apis/evaluate_task2.py \
    --api gemini \
    --problems "${PROBLEMS}" \
    --frames_dir "${FRAMES_DIR}" \
    --output "./results/task2_gemini.json" \
...
printf "All Task2 API evaluations completed.\n\n"
