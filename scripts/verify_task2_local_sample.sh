...
# ──────────────────────────────────────────────────────────────────────────────
# Verify Task 2 local evaluation on a small sample (1 problem, 2-3 turns).
# Runs local HuggingFace models, prints per-turn scores side by side.
# Usage:
#   ./scripts/verify_task2_local_sample.sh            # default: problem idx 0
#   SAMPLE_IDX=1 ./scripts/verify_task2_local_sample.sh  # pick another problem
#   MODELS="qwen2p5_vl_7b" ./scripts/verify_task2_local_sample.sh  # single model
#   MODELS="all" ./scripts/verify_task2_local_sample.sh  # include llama4
# ──────────────────────────────────────────────────────────────────────────────
...
OUT_DIR="./results/verify_local_sample"
mkdir -p "$OUT_DIR"

echo "============================================================"
echo " Task 2 Local — Sample Verification"
...
for tag in model_tags:
    path = os.path.join(out_dir, f"task2__{tag}.json")
...
for tag in model_tags:
    if tag in data:
        ...
        # Use short name: last part of HF id
        short = hf_id.split("/")[-1] if "/" in hf_id else hf_id
        label = f"{tag}"
