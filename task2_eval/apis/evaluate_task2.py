#!/usr/bin/env python3
"""
Task 2 (Wooden Puzzle) multi-turn evaluation via API models.
...
def build_turn0_prompt(header: Dict[str, Any]) -> str:
    return (
        ...
        "The second image is the Block Reference showing all 16 blocks with their labels.\n"
...
def build_turn_prompt(turn_num: int, is_turn1: bool,
    ...
    lines.append(f"Q1. Manipulated Block: Which block was manipulated in this turn? (Select one)")
    lines.append(f"Options: {q1_str}\n")

    if not is_turn1:
        lines.append(f"Q2. Latest Manipulated Block: Which block was the latest one manipulated before this turn? (Select one)")
        lines.append(f"Options: {q2_str}\n")

    lines.append(f"Q3. Contact Blocks: Which block(s) are in contact with the manipulated block? "
                 f"Only count side-face-to-side-face contact. Partial contacts from vertical stacking count. (Select all that apply)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Task 2 Wooden Puzzle evaluation (API)")
    parser.add_argument("--problems", default="./problems/wooden_puzzle.json")
    ...
    out_path = Path(args.output) if args.output else Path(f"./results/task2_{args.api}_{model_name.replace('/', '_')}.json")
