#!/usr/bin/env python3
"""
Task 2 (Wooden Puzzle) multi-turn evaluation via local HuggingFace models.
...
def build_turn_prompt(turn_num: int, is_turn1: bool,
    ...
    lines.append("Q3. Contact Blocks: Which block(s) are in contact with the manipulated block? "
                 "Only count side-face-to-side-face contact. Partial contacts from vertical stacking count. (Select all that apply)")
...
def main():
    ap = argparse.ArgumentParser(description="Task 2 Wooden Puzzle evaluation (local)")
    ap.add_argument("--problems", default="./problems/wooden_puzzle.json")
    ...
    for tag, hf_id in model_entries:
        ...
        for i in range(total_problems):
            ...
            # Incremental save
            out_path = out_dir / f"task2__{tag}{args.output_suffix}.json"
        ...
        out_path = out_dir / f"task2__{tag}.json"
        out_path.write_text(json.dumps(
